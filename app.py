...
conn = sqlite3.connect('nivi.db', check_same_thread=False)
cursor = conn.cursor()

# Create table for user details if not exists
cursor.execute('''
...
def submit_user_details():
    try:
        ...
        cursor.execute('SELECT * FROM users WHERE email = ? AND scores IS NOT NULL', (email,))
        ...
        cursor.execute('INSERT INTO users (name, email, age, gender) VALUES (?, ?, ?, ?)',
                       (user_details['name'], user_details['email'], user_details['age'], user_details['gender']))
        conn.commit()
        conn.close()
...
def submit_responses():
    ...
    if not ngrok_url:
        raise Exception("ngrok tunnel is not started")
    plot_buffer = fetch_plot_image(ngrok_url )
...
def generate_pdf_report(user_details, suggestions, plot_buffer):
    try:
        ...
        pdf.image(temp_plot_file_path, x=image_x, y=image_y, w=image_w, h=image_h)
...
def download_report():
    try:
        suggestions = process_responses(responses)
        plot_buffer = fetch_plot_image(ngrok_url="http://localhost:5000")
        pdf_buffer = generate_pdf_report(user_details, suggestions, plot_buffer)
        email = user_details.get('email')
        if not email:
...
def plot():
    scores = calculate_scores()
    email = user_details.get('email')
    score = str(scores)  # Convert to string if necessary
    ...
    if existing_user:
        ...
        cursor.execute('UPDATE users SET scores = ? WHERE email = ?', (score, email))
    else:
        # Insert a new user record if they don't exist
        cursor.execute('INSERT INTO users (name, email, age, gender, scores) VALUES (?, ?, ?, ?, ?)',
                       (user_details['name'], email, user_details['age'], user_details['gender'], score))
...
def fetch_plot_image(ngrok_url="localhost:5000"):
    try:
        response = requests.get(f'{ngrok_url}/plot')
...
def show_results():
    ...
    return render_template_string(open('result.html').read(), user_details=dict(user))
otp_store = {}
...
if __name__ == '__main__':
    ...
    print(f" * ngrok tunnel \ -> \"http://127.0.0.1:5000\"")
    app.run(port=5000)